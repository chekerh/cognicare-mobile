...
Sois chaleureux, bienveillant et encourageant. Réponds dans la langue de l'utilisateur. Sois concis (2-4 phrases max sauf si demandé). Ne donne jamais de diagnostic médical.`;
    ...
    const tools = [
      {
        ...
        function: {
          ...
          description:
            "Ajoute une nouvelle tâche ou un rappel dans l'agenda quotidien de l'enfant.",
          parameters: {
            type: 'object',
            properties: {
              childId: {
                type: 'string',
                description:
                  "L'ID de l'enfant (ex: 698f66198ac...). IMPORTANT: doit être un ID valide de la liste.",
              },
              title: {
                type: 'string',
                description: "Titre clair de la tâche, ex: 'Faire du sport'",
              },
              time: {
                type: 'string',
                description: "Heure au format HH:mm, ex: '09:30'",
              },
            },
      ...
      for (const toolCall of responseMessage.tool_calls) {
        if (toolCall.function.name === 'add_routine_task') {
          try {
            ...
            const result = await this.executeAddRoutineTask(
              userId,
              args.childId,
              args.title,
              args.time,
  ...
  private async executeAddRoutineTask(
    userId: string,
    childId: string,
    title: string,
    time: string,
    ...
    try {
      ...
      const childExists = await this.childModel
        .findOne({ _id: childId, parentId: userId })
        .lean()
        .exec();
      if (!childExists) {
        return JSON.stringify({
          success: false,
          error:
            "L'enfant spécifié n'a pas été trouvé ou n'appartient pas à l'utilisateur.",
        });
      }
      ...
      return JSON.stringify({
        success: true,
        message: `Tâche '${title}' ajoutée à ${time} pour ${childExists.fullName}.`,
      });
    } catch (error: any) {
