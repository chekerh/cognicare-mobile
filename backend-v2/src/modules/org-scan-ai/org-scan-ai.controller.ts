...
export class OrgScanAiController {
  ...
  async rejectAnalysis(
    ...
    if (pendingOrg && pendingOrg.requestedBy) {
      try {
        ...
        this.orgScanAiService["logger"].error(
  ...
  async approveAnalysis(
    ...
    if (!result) throw new BadRequestException("Analysis not found");
    return { message: "Analysis marked as approved", analysis: result };
  }
  ...
  @Roles("admin")
  @ApiBearerAuth()
  @ApiOperation({ summary: "Get pending organization details" })
  ...
  async analyzeDocumentLegacy(
    ...
    if (result.riskScore > 0.7) riskLevel = "HIGH";
    else if (result.riskScore > 0.4) riskLevel = "MEDIUM";
    else riskLevel = "LOW";
    return {
  ...
  @Roles("admin")
  @ApiBearerAuth()
  @ApiOperation({ summary: "Rescan certificate for a pending organization" })
  @ApiResponse({ status: 200, type: FraudAnalysisResponse })
  async rescanPendingOrganization(
    ...
    if (!pendingOrg.certificateUrl)
      throw new BadRequestException("No certificate URL found");
    ...
    try {
      ...
      if (!pdfHeader.startsWith("%PDF-")) {
        ...
        if (hexHeader.startsWith("89504E47")) detectedType = "PNG image";
        else if (hexHeader.startsWith("FFD8FF")) detectedType = "JPEG image";
        else if (hexHeader.startsWith("47494638")) detectedType = "GIF image";
        throw new BadRequestException(
          `Certificate is not a valid PDF (detected: ${detectedType}). Reject and ask for re-registration.`,
        );
      }
    } catch (error) {
      if (error instanceof BadRequestException) throw error;
      const msg = error instanceof Error ? error.message : "Unknown error";
      throw new BadRequestException(`Failed to download certificate: ${msg}`);
    }
  ...
  @ApiConsumes("multipart/form-data")
  @ApiOperation({ summary: "Re-upload certificate for a pending organization" })
  @ApiBody({
    schema: {
      type: "object",
      properties: {
        certificate: {
          type: "string",
          format: "binary",
          description: "Valid PDF certificate",
        },
      },
  ...
  async reuploadCertificate(
    ...
    if (certificate.mimetype !== "application/pdf")
      throw new BadRequestException("Certificate must be a PDF");
    const pdfHeader = certificate.buffer.toString("utf8", 0, 5);
    if (!pdfHeader.startsWith("%PDF-"))
      throw new BadRequestException("Invalid PDF header");
    if (certificate.size > 10 * 1024 * 1024)
      throw new BadRequestException("Max 10MB");
