...
export class RiskService {
  ...
  calculateFraudRisk(
    ...
    if (fraudRisk >= 0.6) level = "HIGH";
    else if (fraudRisk >= 0.3) level = "MEDIUM";
    else level = "LOW";

    const flags: string[] = [...additionalFlags];
    if (documentScore > 0.5) flags.push("High document inconsistency");
    if (similarityScore > 0.85)
      flags.push("High similarity to previous submission");
    if (domainScore > 0.3) flags.push("Domain risk factors detected");
