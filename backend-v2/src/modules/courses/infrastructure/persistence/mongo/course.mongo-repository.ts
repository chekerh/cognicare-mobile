...
import { CourseMapper, CourseEnrollmentMapper } from '../../mappers/course.mapper';
...
export class CourseMongoRepository implements ICourseRepository {
  ...
  async findAll(filters?: { qualificationOnly?: boolean; courseType?: string; hasCertification?: boolean }): Promise<CourseEntity[]> {
    const q: Record<string, unknown> = {};
    if (filters?.qualificationOnly) q.isQualificationCourse = true;
    if (filters?.courseType) q.courseType = filters.courseType;
    if (filters?.hasCertification) q.certification = { $exists: true, $nin: [null, ''] };
    return (await this.model.find(q).sort({ createdAt: 1 }).lean().exec()).map(CourseMapper.toDomain);
  }
...
export class CourseEnrollmentMongoRepository implements ICourseEnrollmentRepository {
  constructor(@InjectModel('CourseEnrollment') private readonly model: Model<any>) {}

  async findByUserId(userId: string): Promise<CourseEnrollmentEntity[]> {
    return (await this.model.find({ userId: new Types.ObjectId(userId) }).sort({ updatedAt: -1 }).lean().exec()).map(CourseEnrollmentMapper.toDomain);
  }

  async findByUserAndCourse(userId: string, courseId: string): Promise<CourseEnrollmentEntity | null> {
    const doc = await this.model.findOne({ userId: new Types.ObjectId(userId), courseId: new Types.ObjectId(courseId) }).lean().exec();
  ...
  async findAll(userId?: string): Promise<CourseEnrollmentEntity[]> {
    ...
    return (await this.model.find(q).sort({ updatedAt: -1 }).lean().exec()).map(CourseEnrollmentMapper.toDomain);
  }

  async findCompletedByUser(userId: string): Promise<CourseEnrollmentEntity[]> {
    return (await this.model.find({ userId: new Types.ObjectId(userId), status: 'completed', progressPercent: 100 }).lean().exec()).map(CourseEnrollmentMapper.toDomain);
  }
  ...
  async update(entity: CourseEnrollmentEntity): Promise<CourseEnrollmentEntity> {
    const data = CourseEnrollmentMapper.toPersistence(entity);
    const { _id, ...rest } = data;
