...
export const COURSE_ENROLLMENT_REPOSITORY_TOKEN = Symbol(
  "ICourseEnrollmentRepository",
);
...
export class CreateCourseUseCase {
  constructor(
    @Inject(COURSE_REPOSITORY_TOKEN) private readonly repo: ICourseRepository,
  ) {}
  async execute(dto: {
    title: string;
    description?: string;
    ...
    const entity = CourseEntity.create({
      ...dto,
      isQualificationCourse: dto.isQualificationCourse ?? false,
    });
  ...
  private toOutput(c: CourseEntity) {
    return {
      ...
      slug: c.slug,
      isQualificationCourse: c.isQualificationCourse,
...
export class EnrollCourseUseCase {
  constructor(
    ...
    @Inject(COURSE_ENROLLMENT_REPOSITORY_TOKEN)
    private readonly enrollRepo: ICourseEnrollmentRepository,
  ) {}
  async execute(
    userId: string,
    courseId: string,
    ...
    const existing = await this.enrollRepo.findByUserAndCourse(
      userId,
      courseId,
    );
    if (!existing) {
...
export class MyEnrollmentsUseCase {
  constructor(
    @Inject(COURSE_ENROLLMENT_REPOSITORY_TOKEN)
    private readonly repo: ICourseEnrollmentRepository,
  ) {}
