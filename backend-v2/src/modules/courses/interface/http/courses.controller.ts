...
export class CoursesController {
  ...
  async create(@Body() body: {
    ...
    startDate?: string; endDate?: string; courseType?: string; price?: string;
    location?: string; enrollmentLink?: string; certification?: string;
    targetAudience?: string; prerequisites?: string; sourceUrl?: string;
  }) {
    const dto = { ...body, startDate: body.startDate ? new Date(body.startDate) : undefined, endDate: body.endDate ? new Date(body.endDate) : undefined };
  ...
  async enroll(@Request() req: { user: { id: string } }, @Param('id') courseId: string) {
  ...
  async updateProgress(
    ...
    const result = await this.updateProgressUC.execute(req.user.id, enrollmentId, value);
    if (result.isFailure) throw new NotFoundException(result.error);
    return result.value;
  }
}
