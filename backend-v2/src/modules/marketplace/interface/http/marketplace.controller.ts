...
import { CreateProductDto, CreateReviewDto } from '../../application/dto/marketplace.dto';
import {
...
export class MarketplaceController {
  ...
  async uploadImage(@UploadedFile() file?: { buffer: Buffer; mimetype: string }) {
    ...
    const result = await this.uploadImageUC.execute({ buffer: file.buffer, mimetype: file.mimetype });
    if (result.isFailure) throw new BadRequestException(result.error);
  ...
  async create(
    ...
    const result = await this.createProductUC.execute({ userId: req.user.id, dto });
    if (result.isFailure) throw new BadRequestException(result.error);
    return result.value;
  }
}
