...
export class IntegrationsService implements OnModuleInit {
  ...
  async submitOrder(
    ...
    try {
      if (websiteSlug === BIOHERBS_SLUG) {
        ...
        if (product) {
          this.logger.log(
            `BioHerbs: submitting order ${order._id} via Puppeteer (variant ${product.variantId})`,
          );
          ...
          if (result.success) {
            ...
            this.logger.warn(
              `BioHerbs Puppeteer failed for order ${order._id}. Error: ${result.error}`,
            );
          }
          if (customerEmail) {
          ...
          this.logger.warn(
            `BioHerbs: product handle "${payload.externalId}" not found, skipping Puppeteer`,
          );
          if (customerEmail) {
            await this.mailService.sendBioherbsOrderConfirmationToCustomer(
              customerEmail,
  ...
  private async sendOrderToExternalSite(
    ...
    this.logger.log(
      `Order ${order._id} sent to site ${website.slug} (${formActionUrl})`,
    );
  }
}
