/**
 * Puppeteer automation for submitting orders on BioHerbs
 * so that customers receive the real BioHerbs confirmation email.
 */
...
export async function submitBioherbsOrderWithPuppeteer(
  ...
  try {
    ...
    // 1) Add to cart via Shopify API in page context (same origin)
    await page
      .goto(BIOHERBS_BASE, { waitUntil: "networkidle2" })
      .catch(() => {});
    ...
    await page
      .goto(`${BIOHERBS_BASE}/checkout`, { waitUntil: "networkidle2" })
      .catch(() => {});
    ...
    try {
      await page
        .select('select[name="checkout[shipping_address][country]"]', "TN")
        .catch(() => {});
    } catch {
      // may be pre-filled
    }

    // 6) Select "Cash on delivery" if available
    await new Promise((r) => setTimeout(r, 1000));
    await page.evaluate(() => {
      const labels = Array.from(
        document.querySelectorAll('label, span, div[role="button"]'),
      );
    ...
    // 7) Click through steps to "Complete order"
    const continueButtonTexts = ["Continuer", "Continue", "Next", "Suivant"];
    const completeOrderTexts = [
      "Passer la commande",
      "Complete order",
      "Commander",
      "Pay now",
    ...
    // 8) Check for thank-you / order confirmation
    ...
    return {
      success: false,
      error:
        "Confirmation de commande non détectée (étape paiement ou bouton final non atteinte)",
      externalOrderId,
    };
  } catch (err) {
