...
export async function fetchBioherbsProducts(
  page = 1,
  limit = 50,
): Promise<{ products: BioherbsProduct[]; nextPage: number | null }> {
...
export async function fetchAllBioherbsProducts(): Promise<BioherbsProduct[]> {
  ...
  while (nextPage) {
    const result = await fetchBioherbsProducts(page, 50);
...
export async function fetchBioherbsProductByHandle(handle: string): Promise<{
  ...
  const res = await axios
    .get<{ product?: ShopifyProductJson }>(url, {
      timeout: 10000,
      headers: { Accept: "application/json" },
    })
  ...
  return { handle: p.handle, title: p.title, variantId: v.id, price: v.price };
}

export async function addBioherbsToCart(
  variantId: number,
  quantity: number,
): Promise<string> {
