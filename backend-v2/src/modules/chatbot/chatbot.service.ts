...
Sois chaleureux et concis (2-4 phrases). Ne donne jamais de diagnostic médical.`;
  ...
  private async tryProviders(
    ...
    if (this.openaiApiKey) {
      try {
        ...
        throw new Error(`Désolé ${userName}, services AI indisponibles.`);
      }
    }
  ...
  private async executeAddRoutineTask(
    ...
    const child = await this.childModel
      ...
      .exec();
    if (!child)
      return JSON.stringify({ success: false, error: "Enfant non trouvé." });
    ...
    return JSON.stringify({
      success: true,
      message: `Tâche '${title}' ajoutée à ${time} pour ${(child as any).fullName}.`,
    });
  }
}
