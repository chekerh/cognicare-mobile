...
export class PaypalService {
  ...
  async createOrder(params: {
    amount: string; currencyCode?: string; returnUrl: string; cancelUrl: string;
  }): Promise<{ orderId: string; approvalUrl: string }> {
    const token = await this.getAccessToken();
    const body = {
      intent: 'CAPTURE',
      purchase_units: [{ amount: { currency_code: params.currencyCode ?? 'USD', value: params.amount } }],
    ...
    if (!approveLink?.href) throw new Error('PayPal order created but no approval link');
  ...
  async captureOrder(orderId: string): Promise<{ status: string; id?: string }> {
    ...
    return { status: data.status ?? 'COMPLETED', id: data.id ?? orderId };
  }
