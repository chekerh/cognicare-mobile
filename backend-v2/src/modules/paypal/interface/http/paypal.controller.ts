import {
  Controller, Get, Post, Body, Query, Res, BadRequestException,
} from '@nestjs/common';
...
export class PaypalController {
  ...
  async createOrder(@Body() body: { amount: string; currencyCode?: string }) {
    if (!this.paypal.isConfigured()) throw new BadRequestException('PayPal is not configured');
    const amount = body.amount ?? (body as Record<string, any>)['value'];
    if (!amount || typeof amount !== 'string') throw new BadRequestException('amount is required (e.g. "75.00")');
    const base = process.env.PAYPAL_RETURN_BASE || process.env.BASE_URL || 'http://localhost:3000';
    const returnUrl = `${base.replace(/\/$/, '')}/api/v1/paypal/complete`;
    const cancelUrl = `${base.replace(/\/$/, '')}/api/v1/paypal/cancel`;
    return this.paypal.createOrder({ amount, currencyCode: body.currencyCode ?? 'USD', returnUrl, cancelUrl });
  }

  @Get('complete')
  @Public()
  @ApiOperation({ summary: 'PayPal redirects here after approval' })
  async complete(@Query('token') token: string | undefined, @Res() res: Response) {
    if (!token) {
    ...
    try {
      ...
      res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="2;url=${redirectUrl}"></head><body><p>Payment successful. Redirecting...</p></body></html>`);
    } catch (e) {
      res.status(500).send(`<html><body><p>Error: ${(e as Error).message}</p></body></html>`);
    }
  }
  ...
  cancel(@Res() res: Response) {
    ...
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="2;url=${redirectUrl}"></head><body><p>Payment cancelled.</p></body></html>`);
  }
}
