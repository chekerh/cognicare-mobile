...
{"status":"VALID"|"UNCERTAIN"|"INVALID","reasoning":"message simple","metadata":{"medicineName":"...","dosage":"...","expiryDate":"...","isExpired":true|false|null,"manufacturer":"...","detailedDescription":"..."}}`;
    ...
    try {
      ...
      if (!fs.existsSync(fullPath)) {
        return {
          status: "UNCERTAIN",
          reasoning: "Fichier image introuvable.",
          metadata: {
            medicineName: "N/A",
            dosage: "N/A",
            expiryDate: "N/A",
      ...
      if (!aiResult)
        return {
          status: "UNCERTAIN",
          reasoning: "Réponse IA non interprétée.",
          metadata: {
            medicineName: "N/A",
            dosage: "N/A",
            expiryDate: "N/A",
      ...
      if (
        ...
        !["N/A", "Non visible"].includes(aiResult.metadata.expiryDate)
      ) {
  ...
  private checkExpiryDate(dateStr: string): boolean | null {
    try {
      ...
      if (m1) {
        ...
        if (y < 100) y += 2000;
        return new Date(y, parseInt(m1[1], 10), 0) < now;
      }
      const m2 = dateStr.match(/(\d{4})[/-](\d{1,2})/);
      if (m2)
        return new Date(parseInt(m2[1], 10), parseInt(m2[2], 10), 0) < now;
      return null;
    } catch {
      return null;
    }
  ...
  private async validateWithFDA(drugName: string): Promise<any> {
    ...
    try {
      const r = await axios.get<{ results?: unknown[] }>(
        `https://api.fda.gov/drug/label.json?search=openfda.brand_name:"${encodeURIComponent(drugName)}"&limit=1${suffix}`,
        { timeout: 5000 },
      );
      ...
      try {
        const r = await axios.get<{ results?: unknown[] }>(
          `https://api.fda.gov/drug/label.json?search=openfda.generic_name:"${encodeURIComponent(drugName)}"&limit=1${suffix}`,
          { timeout: 5000 },
        );
        if (r.data.results?.length) return r.data.results[0];
      } catch {
