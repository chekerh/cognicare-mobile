...
Respond with a valid JSON object in this exact format (no markdown, no code block):
{
  ...
  "recommendations": [
    { "planType": "PECS", "text": "Specific, actionable recommendation for PECS." },
  ...
  "predictions": "1–3 sentences with progress forecasting for each relevant plan type."
}

Rules:
- Provide exactly one recommendation per plan type that appears in the plans array; use brief placeholder text only if no data for that type.
- Always include milestones and predictions when any plan data exists.
- Use short, actionable language.`;
  ...
  private async callModel(prompt: string): Promise<string> {
    ...
    this.logger.error(
      `All models failed (404). Tried: ${modelsToTry.join(", ")}.`,
    );
    throw new Error(
      "Gemini API: model not found (404). Check PROGRESS_AI_MODEL and API key.",
    );
  }
