...
import { SpecializedPlansService } from "@/modules/specialized-plans/specialized-plans.service";
import { LlmService, LlmPreferences, LlmRecommendation } from "./llm.service";
import { GetCompletionStatsUseCase } from "@/modules/nutrition/application/use-cases/nutrition.use-cases";
...
export class ProgressAiService {
  ...
  constructor(
    ...
    @InjectModel("TaskReminder")
    private taskReminderModel: Model<any>,
    private getCompletionStatsUC: GetCompletionStatsUseCase,
  ...
  async getRecommendations(
    ...
    try {
      ...
      if (e instanceof NotFoundException || e instanceof ForbiddenException)
        throw e;
      this.logger.warn(
        `buildContext failed childId=${childId}: ${(e as Error)?.message}`,
      );
    ...
    try {
      ...
      this.logger.warn(
        `LLM failed childId=${childId}: ${(e as Error)?.message}`,
      );
      return this.fallbackRecommendationResult();
    }
  ...
  private fallbackRecommendationResult(): RecommendationResult {
    return {
      ...
      summary:
        "Recommandations temporairement indisponibles. Vérifiez que le service IA (GEMINI_API_KEY) est configuré.",
      recommendations: [],
  ...
  async getParentSummary(
    ...
    const stats = (await this.getCompletionStatsUC.execute(childId, days)) as {
      totalTasks: number;
