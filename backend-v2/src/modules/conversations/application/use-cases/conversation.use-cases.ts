...
// ── Get Inbox ──
@Injectable()
export class GetInboxUseCase implements IUseCase<
  string,
  Result<any[], string>
> {
  constructor(
...
@Injectable()
export class GetOrCreateConversationUseCase implements IUseCase<
  { userId: string; otherUserId: string; userName: string; otherName: string },
  Result<any, string>
> {
  constructor(
    @Inject(CONVERSATION_REPOSITORY_TOKEN)
  ...
  async execute(input: {
    ...
    const conv = await this.repo.findByUserAndOther(
      input.userId,
      input.otherUserId,
    );
...
export class CreateGroupUseCase implements IUseCase<
  { userId: string; name: string; imageUrl?: string; participantIds: string[] },
  Result<any, string>
> {
  constructor(
    @Inject(CONVERSATION_REPOSITORY_TOKEN)
...
// ── Add Member To Group ──
@Injectable()
export class AddMemberToGroupUseCase implements IUseCase<
  { conversationId: string; userId: string },
  Result<void, string>
> {
  constructor(
...
// ── Get/Update Settings ──
@Injectable()
export class GetSettingsUseCase implements IUseCase<
  { userId: string; conversationId: string },
  Result<any, string>
> {
  constructor(
    @Inject(CONVERSATION_SETTING_REPOSITORY_TOKEN)
    private readonly repo: IConversationSettingRepository,
  ) {}
  async execute(input: {
    userId: string;
    conversationId: string;
  }): Promise<Result<any, string>> {
    const setting = await this.repo.findByUserAndConversation(
      input.userId,
...
@Injectable()
export class UpdateSettingsUseCase implements IUseCase<
  {
    userId: string;
    conversationId: string;
    data: { autoSavePhotos?: boolean; muted?: boolean };
  },
  Result<any, string>
> {
  constructor(
  ...
  async execute(input: {
    ...
    let setting = await this.repo.findByUserAndConversation(
      input.userId,
      input.conversationId,
    );
    if (setting) {
    ...
    setting = ConversationSettingEntity.create({
      ...
      autoSavePhotos: input.data.autoSavePhotos ?? false,
      muted: input.data.muted ?? false,
    });
...
// ── Get Media ──
@Injectable()
export class GetMediaUseCase implements IUseCase<
  { conversationId: string; userId: string },
  Result<any[], string>
> {
  constructor(
...
// ── Search Messages ──
@Injectable()
export class SearchMessagesUseCase implements IUseCase<
  { conversationId: string; userId: string; query: string },
  Result<any[], string>
> {
  constructor(
    @Inject(CONVERSATION_REPOSITORY_TOKEN)
...
// ── Upload Attachment ──
@Injectable()
export class UploadAttachmentUseCase implements IUseCase<
