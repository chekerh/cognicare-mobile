...
export class CallsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleCallInitiate(
    ...
    this.emitToUser(payload.targetUserId, "call:incoming", {
  ...
  handleCallAccept(
    ...
    if (!client.userId) return;
    this.emitToUser(payload.fromUserId, "call:accepted", {
      channelId: payload.channelId,
    });
  }
  ...
  handleWebRTCOffer(
    ...
    if (!client.userId) return;
    this.emitToUser(payload.targetUserId, "webrtc:offer", {
      fromUserId: client.userId,
      sdp: payload.sdp,
      type: payload.type,
    });
  ...
  handleWebRTCAnswer(
    ...
    if (!client.userId) return;
    this.emitToUser(payload.targetUserId, "webrtc:answer", {
      fromUserId: client.userId,
      sdp: payload.sdp,
      type: payload.type,
    });
  ...
  handleICE(
    ...
    if (!client.userId) return;
    this.emitToUser(payload.targetUserId, "webrtc:ice-candidate", {
      fromUserId: client.userId,
  ...
  handleTyping(
    ...
    if (!client.userId) return;
    this.emitToUser(payload.targetUserId, "chat:typing", {
