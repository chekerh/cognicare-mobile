...
export class TranscriptionService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (!apiKey) {
      this.logger.warn("DEEPGRAM_API_KEY not found. Transcription disabled.");
      return;
    }
    try {
      ...
      this.logger.error(`Failed to init Deepgram: ${error.message}`);
    }
  }
  ...
  createStream(callbacks: {
    ...
    if (!this.deepgram) {
      this.logger.error("Deepgram not initialized");
      return null;
    }
    try {
