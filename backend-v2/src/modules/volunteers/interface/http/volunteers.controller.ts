...
import {
  ...
  AssignTaskDto,
} from "../../application/dto/volunteer.dto";
...
export class VolunteersController {
  constructor(
    ...
    @Inject("CLOUDINARY_UPLOAD_FN")
    private readonly uploadFn: (
      buffer: Buffer,
      options: any,
    ) => Promise<string>,
    @Inject("HAS_COMPLETED_QUALIFICATION_FN")
    private readonly hasCompletedQualificationFn: (
      userId: string,
    ) => Promise<boolean>,
    @Inject("SEND_VOLUNTEER_EMAIL_FN")
  ...
  async completeCertification(@Req() req: any) {
    return this.completeCert.execute(req.user.userId, () =>
