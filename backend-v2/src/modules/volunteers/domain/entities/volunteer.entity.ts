...
export class VolunteerApplicationEntity extends Entity<string> {
  ...
  static create(userId: string): VolunteerApplicationEntity {
    return new VolunteerApplicationEntity(Entity.generateId(), { userId, status: 'pending', documents: [] });
  }

  static reconstitute(id: string, props: VolunteerApplicationProps): VolunteerApplicationEntity {
    return new VolunteerApplicationEntity(id, props);
  }

  get userId() { return this.props.userId; }
  get status() { return this.props.status; }
  get careProviderType() { return this.props.careProviderType; }
  get specialty() { return this.props.specialty; }
  get organizationName() { return this.props.organizationName; }
  get organizationRole() { return this.props.organizationRole; }
  get documents() { return this.props.documents; }
  get deniedReason() { return this.props.deniedReason; }
  get reviewedBy() { return this.props.reviewedBy; }
  get reviewedAt() { return this.props.reviewedAt; }
  get trainingCertified() { return this.props.trainingCertified ?? false; }
  get trainingCertifiedAt() { return this.props.trainingCertifiedAt; }
  get denialNotificationSent() { return this.props.denialNotificationSent ?? false; }
  get profileComplete() { return this.props.documents.length >= 1; }
  get createdAt() { return this.props.createdAt; }
  get updatedAt() { return this.props.updatedAt; }

  updateProfile(dto: { careProviderType?: string; specialty?: string; organizationName?: string; organizationRole?: string }): void {
    if (this.props.status !== 'pending') throw new Error('Cannot update after review');
    if (dto.careProviderType !== undefined) this.props.careProviderType = dto.careProviderType;
    if (dto.specialty !== undefined) this.props.specialty = dto.specialty;
    if (dto.organizationName !== undefined) this.props.organizationName = dto.organizationName;
    if (dto.organizationRole !== undefined) this.props.organizationRole = dto.organizationRole;
  }

  addDocument(doc: VolunteerDocProps): void {
    if (this.props.status !== 'pending') throw new Error('Cannot add documents after review');
    this.props.documents.push(doc);
  }

  removeDocument(index: number): VolunteerDocProps {
    if (this.props.status !== 'pending') throw new Error('Cannot remove documents after review');
    if (index < 0 || index >= this.props.documents.length) throw new Error('Invalid document index');
...
export class VolunteerTaskEntity extends Entity<string> {
  ...
  static create(props: Omit<VolunteerTaskProps, 'status' | 'createdAt' | 'updatedAt'>): VolunteerTaskEntity {
    return new VolunteerTaskEntity(Entity.generateId(), { ...props, status: 'pending' });
  }

  static reconstitute(id: string, props: VolunteerTaskProps): VolunteerTaskEntity {
    return new VolunteerTaskEntity(id, props);
  }

  get assignedBy() { return this.props.assignedBy; }
  get volunteerId() { return this.props.volunteerId; }
