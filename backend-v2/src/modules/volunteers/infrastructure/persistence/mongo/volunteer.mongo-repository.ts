...
import { VolunteerApplicationMapper, VolunteerTaskMapper } from '../mappers/volunteer.mapper';
...
export class VolunteerApplicationMongoRepository implements IVolunteerApplicationRepository {
  ...
  async findByUserId(userId: string): Promise<VolunteerApplicationEntity | null> {
  ...
  async findAll(filters?: { status?: string }): Promise<VolunteerApplicationEntity[]> {
    const query: any = {};
  ...
  async save(entity: VolunteerApplicationEntity): Promise<VolunteerApplicationEntity> {
  ...
  async update(entity: VolunteerApplicationEntity): Promise<VolunteerApplicationEntity> {
    const data = VolunteerApplicationMapper.toPersistence(entity);
    const doc = await this.model.findByIdAndUpdate(
      entity.id,
      { $set: data },
