import {
  ...
  Body, Param, Query, Request, UseInterceptors, UploadedFile,
  BadRequestException, NotFoundException, ForbiddenException,
} from '@nestjs/common';
import { FileInterceptor } from '@nestjs/platform-express';
import { ApiTags, ApiOperation, ApiResponse, ApiBearerAuth } from '@nestjs/swagger';
import { CreatePostDto, UpdatePostDto, CreateCommentDto } from '../../application/dto/community.dto';
...
export class CommunityController {
  ...
  async updatePost(
    ...
    const result = await this.updatePostUC.execute({ postId, userId: req.user.id, dto });
    if (result.isFailure) {
      if (result.error === 'Post not found') throw new NotFoundException(result.error);
  ...
  async deletePost(
    ...
    const result = await this.deletePostUC.execute({ postId, userId: req.user.id });
    if (result.isFailure) {
      if (result.error === 'Post not found') throw new NotFoundException(result.error);
      throw new ForbiddenException(result.error);
    }
  ...
  async toggleLike(
    ...
    const result = await this.toggleLikeUC.execute({ postId, userId: req.user.id });
  ...
  async getLikeStatus(
    ...
    const result = await this.getLikeStatusUC.execute({ postIds, userId: req.user.id });
    if (result.isFailure) throw new BadRequestException(result.error);
    return result.value;
  }
}
