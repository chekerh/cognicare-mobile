...
import { COMMENT_REPOSITORY_TOKEN, ICommentRepository } from '../../domain/repositories/comment.repository.interface';
...
@Injectable()
export class AddCommentUseCase implements IUseCase<AddCommentInput, Result<CommentOutputDto, string>> {
  constructor(
    @Inject(POST_REPOSITORY_TOKEN) private readonly postRepo: IPostRepository,
    @Inject(COMMENT_REPOSITORY_TOKEN) private readonly commentRepo: ICommentRepository,
  ) {}

  async execute(input: AddCommentInput): Promise<Result<CommentOutputDto, string>> {
