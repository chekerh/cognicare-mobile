...
@Injectable()
export class DeletePostUseCase implements IUseCase<DeletePostInput, Result<void, string>> {
  constructor(
  ...
  async execute(input: DeletePostInput): Promise<Result<void, string>> {
    ...
    if (!post.isAuthor(input.userId)) return err('You can only delete your own posts');
