...
@Injectable()
export class UpdatePostUseCase implements IUseCase<UpdatePostInput, Result<void, string>> {
  constructor(
  ...
  async execute(input: UpdatePostInput): Promise<Result<void, string>> {
    ...
    if (!post.isAuthor(input.userId)) return err('You can only edit your own posts');
